...
namespace Nico.Assistant.Socket
{
    ...
    {
        private readonly INicoService _nicoService;

        protected ChatHubBase(INicoService nicoService)
        {
            this._nicoService = nicoService;     
        }
        public override async Task OnConnectedAsync()
        {
            await base.OnConnectedAsync();
            //cuando se conecta se envia mensaje de bienvenida
            await _nicoService.ProcessMessageStart(async(response)=> 
                {
        ...
        public async Task OnMessageCli(ReceiveMessagePayload callbackmg)
        {
            ...
            await _nicoService.ProcessAndValidateConvesation(conversation, async (response) =>
            {
        ...
        public async Task SendMensage(string idConecction, ResponseNico message)
        {
